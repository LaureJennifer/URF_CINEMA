...
namespace URF_Cinema.Client.Repositories.Implements
{
    public class FilmScheduleRoomRepo : IFilmScheduleRoomRepo
    {
        ...
        public async Task<FilmScheduleRoomListWithPaginationViewModel> GetFilmScheduleRoomByRoomAsync(ViewFilmScheduleRoomWithPaginationRequest request)
        {
            var client = new HttpClient
            {
                BaseAddress = new Uri("https://localhost:7005")
            ...
            var obj = await client.GetFromJsonAsync<FilmScheduleRoomListWithPaginationViewModel>(url);
            if (obj != null)
                return obj;
            return null;
        }

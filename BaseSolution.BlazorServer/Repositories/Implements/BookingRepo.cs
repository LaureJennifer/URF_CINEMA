...
namespace BaseSolution.BlazorServer.Repositories.Implements
{
    ...
    {
        public async Task<BookingListWithPaginationViewModel> GetAllActive(ViewBookingWithPaginationRequest request)
        {
            var client = new HttpClient
            {
                BaseAddress = new Uri("https://localhost:7005")
            };
            var obj = await client.GetFromJsonAsync<BookingListWithPaginationViewModel>($"api/Bookings");
            if (obj != null)
                return obj;
            return new();
        }

...
namespace BaseSolution.BlazorServer.Repositories.Implements
{
    ...
    {
        public async Task<UserCreateRequest> AddAsync(UserCreateRequest request)
        {
            var client = new HttpClient
            {
                BaseAddress = new Uri("https://localhost:7005")
            };
            var obj = await client.PostAsJsonAsync("api/Users", request).Result.Content.ReadAsStringAsync();
            var result = JsonConvert.DeserializeObject<UserCreateRequest>(obj);
            return result;
        }

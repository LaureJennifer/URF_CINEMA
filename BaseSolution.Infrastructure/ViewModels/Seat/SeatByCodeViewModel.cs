...
{
    public class SeatByCodeViewModel:ViewModelBase<string>
    {
        private readonly ISeatReadOnlyRepository _seatReadOnlyRepository;
        private readonly ILocalizationService _localizationService;

        public SeatByCodeViewModel(ISeatReadOnlyRepository seatReadOnlyRepository, ILocalizationService localizationService)
        {
        ...
        public async override Task HandleAsync(string code, CancellationToken cancellationToken)
        {
            try
            {
                var result = await _seatReadOnlyRepository.GetSeatByCodeAsync(code, cancellationToken);
                Data = result.Data!;
